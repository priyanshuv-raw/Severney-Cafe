...
class Bill_App:
    ...
    def bill_area(self):
        if self.c_name.get()=="" or self.c_phone.get()=="" :
            ...
            self.txtarea.insert(END,f"Number of Deserts:              {(self.brownie.get())+(self.chocolate.get())+(self.puddings.get())+(self.waffle.get())+(self.roll.get())+(self.applepie.get())}\n")
    ...
    def save_bill(self):
        ...
        if op>0:
            ...
            f1=open("bill/"+str(self.c_name.get())+"_Bill_No_"+str(self.bill_no.get())+".txt","w")
